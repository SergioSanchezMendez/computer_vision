...
%% BOX - Bounding box proximity (Proximidad a la caja contenedora)
%%
%% Ejemplo de llamada a la función:
%% BOX('C:/Users/Ana/Desktop/VC tracking/test1/imgs2', 0.75, 0.3)


function BOX(folder, alpha, ini_thres)
...
tic; 
boxes = [];
centerList = [];    % Contains for each blob the points of its detected bounding box 
SE = strel('disk', 3);
outm = [];
impad = 5; % Padding around ncc box
ncc_thresh = 0.3; % Threshold for good ncc match
timeave = 0;
distThresh = 17;
		...
		for k = 1:size(dim, 2)
			if dim(j,k) == 1
                ...
                if doit
                        ...
                        % Gets de boundingBox center point
                        c = getCenter(x1, x2, y1, y2);

                        % Gets nearest stored blob. (-1 if 0)
                        [id, dist] = getNearestCenterId(c, centerList);

                        matched = 0;
                        % Have coincidence
                        if(dist >= 0 && dist < distThresh)
                            matched = 1;
                            curcolor = color_list{id};

                            dist;
                            
                            % Update center
                            centerList(id, 1, 1) = c(1);
                            centerList(id, 1, 2) = c(2); 
                        end
                        ...
                        scl = size(centerList); 
                        scl(1);
                        if ( scl(1) < length(color_list)) && ~matched
                           curcolor = color_list{scl(1)+1};         % New color for new element
                           %centerList(scl(1)+1, :) = c(:);         % First center of this blob
                           centerList(scl(1)+1, 1, 1) = c(1);
                           centerList(scl(1)+1, 1, 2) = c(2);

                           curcolor;

                           % Insertion samples. Element
                           % centerList(Z, M, X) = c(2);
                           % centerList(Z, M, Y) = c(1); 
                        end
...
% Returns center from bounding box
function [c] = getCenter(x1, x2, y1, y2)
c(1) = (x2 - x1) / 2;
c(2) = (y2 - y1) / 2;
    


% Returns nearest center Id from list 'centerList' 
%and the Euclidean distance between them. From 'c'
function [id, dist] = getNearestCenterId(c, centerList)
...
% Already with elements    
       
% For each of them extracts lasts center and returns distance if minimum
for i = 1:scl(1)    
    % Last from row
    centerListRow = centerList(i, 1, :);
    
    % Access samples
    % Individual elements. Element 1, Center 1, X -> (1)   
    %                      Element 1, Center 1, Y -> (2)
    % centerListRow(1, 1, 1)
    % centerListRow(1, 1, 2)

    % Individual elements  
    % Element 1, Center 2, X -> (1)   
    % Element 1, Center 2, Y -> (2)     
    % centerListRow(1, 2, 1)
    % centerListRow(1, 2, 2)
    
    lastCenter(1) = centerListRow(1);
    lastCenter(2) = centerListRow(2);
    
    d = sqrt(sum( (c - lastCenter) .^ 2 ));
